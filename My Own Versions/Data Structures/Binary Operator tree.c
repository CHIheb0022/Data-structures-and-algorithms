...
Tree *new_tree(op oper);
void clean_tree(Tree *tr);
Tree *join_tree(Tree *left, Tree *right, op oper);
...
*/
Tree *join_tree(Tree* left, Tree* right, oper op)
{
	Tree *tr = new_tree(op);
...
void print_tree_prefix(Tree *tr)
{
	...
	if(tr->parent != NULL)
    	printf("(%d) -> %d\n", tr->parent->operation, tr->operation);
    else
    	printf("(%d)\n", tr->operation);
